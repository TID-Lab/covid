...
function bodyToFilter(body) {
    const { dates, topic, category, identity, institutions, georgia, platforms, search, } = body || {};
    const filter = {};
    // search
    if (search)
    ...
    if (to) {
        filter.authoredAt = Object.assign(Object.assign({}, filter.authoredAt), { $lte: new Date(to) });
    }
    // topic
    if (topic) {
        filter.topics = topic;
    ...
    const excludesTags = [];
    if (category)
        includesTags.push(category);
    if (identity)
        includesTags.push(identity);
    ...
    if (excludesTags.length > 0) {
        filter.tags = Object.assign(Object.assign({}, filter.tags), { $nin: excludesTags });
    }
    // platform
...
routes.post('/:page', (req, res) => __awaiter(this, void 0, void 0, function* () {
    ...
    const posts = yield postsCollection
        ...
        .limit(pageSize)
        .toArray();
    const lastPage = posts.length === 0 || filteredPostCount - (skipCount + posts.length) <= 0;
    res.status(200).send({
        posts,
        lastPage,
    });

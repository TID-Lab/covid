  ...
  if (pathname === '/social-media-dashboard' || pathname === '/resources') {
    return (
      <header className={`bg-white grid font-medium ${c.header_grid}`}>
        <div className="border-r border-gray-400 px-2 py-2 text-slate-700 flex justify-between items-center">
          <Logo />
          <Menu as="div" className="relative text-left">
            ...
            <Menu.Items className="absolute mt-1 right-0 p-1 z-50 font-medium drop-shadow-lg bg-slate-50 rounded-xs border border-slate-300">
              <Menu.Item>
                {({ active }) => (
                  <button
                    type="button"
                    className={`${
                      active && 'bg-blue-300'
                    } flex items-center gap-x-1 font-medium hover:bg-blue-100 hover:text-blue-800 w-[10rem] rounded-2xs py-1 pr-2 pl-1`}
                    onClick={onLogout}
                  >
                    <Icon type="log-out" size="sm" className="text-red-500" />
                    Logout
                  </button>
                )}
              </Menu.Item>

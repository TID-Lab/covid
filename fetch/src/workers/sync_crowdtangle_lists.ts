...
const {
  fetch: {
    credentials: { instagram: instagramToken, facebook: facebookToken },
  },
} = require('../util/config');
...
async function getRADxListsWith(token) {
  ...
  if (Array.isArray(lists)) {
    return lists
      .filter(
        ({ type, title }) =>
          type === 'LIST' && title.toLowerCase().startsWith('radx')
      )
      .map(({ id, title: name }) => ({
        id,
        name: name.slice(5).toLowerCase(),
      }));
  }
  return [];
}

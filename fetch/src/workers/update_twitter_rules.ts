  ...
  return new Promise((resolve, reject) => {
    ...
    twitterV1.get(
      ...
      (err, data) => {
        ...
        resolve(data.users.map((user) => user.screen_name));
      }
    );
  });
}
...
function generateRuleExpressions(accounts, charsLeft, rulesLeft) {
  ...
  const maxRules1 = Math.ceil(
    (totalChars + ((5 + 4) * accounts.length - 4) + 2) / (charsLeft * 1.0)
  );

  // maximum rules computed by number of "positive clauses"
  const maxRules2 = Math.ceil(
    (accounts.length * 1.0) /
      (MAX_POSITIVE_CLAUSE_COUNT - COVID_KEYWORDS.length)
  );
...
function generateRules(listsByName) {
  ...
  while (i < names.length && rules.length < MAX_RULE_COUNT) {
    ...
    const expressions = generateRuleExpressions(
      accounts,
      charsLeft,
      MAX_RULE_COUNT - rules.length
    );
